...
import { Select, Switch, Radio, message } from "antd";
...
const EditFormField = ({ code, setEditOpen, leadGetApi }) => {
  ...
  const callPutApi = () => {
    ...
    editLeadFormFeildService(payload, code)
      .then((response) => {
  ...
  useEffect(() => {
    {
      getLeadFormFeildDetailService(code)
        .then((response) => {
          const leadData = response.data.data;
          setFormData({
            label: { value: leadData.label },
            code: { value: leadData.code },
            help_text: { value: leadData.help_text },
            placeholder: { value: leadData.placeholder },
            type: { value: leadData.type },
            options: { value: leadData.options },
            is_active: { value: leadData.is_active },
            is_required: { value: leadData.is_required },
            position: { value: leadData.position },
          });
        })
    }
  }, [code]);
