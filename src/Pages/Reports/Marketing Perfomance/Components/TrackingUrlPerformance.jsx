...
import { getTrackingUrlStatusQuery } from "../Queries/MarketingPerfomanceQueries";
import { YgApi } from "../../../../lib/Constants";
dayjs.extend(utc);
dayjs.extend(timezone);

function TrackingUrlPerformance({ startDate, endDate, counsellor, leadSource, sourceGroup, branch, role }) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const query = getTrackingUrlStatusQuery({
                whereClause,
                statuses,
                type: activeTab === "referral" ? "referral_url" : "tracking_url"
            });
            ...
            const dynamicColumns = [
                {
                    title: activeTab === "referral" ? "Referral URL" : "Tracking URL",
                    dataIndex: activeTab === "referral" ? "referral_url" : "tracking_url",
                    fixed: "left",
                    width: 250,
                    align: "left",
                    render: (text) => (
...

                {
                    ...
                    fixed: "left",
                    render: (_, row) => {
                        const percent = getMqlPercent(row.mql, row.total_lead);
...

                {
                    ...
                    fixed: "left",
                    render: (_, row) => {
                        const percent = getMqlPercent(row.sql, row.total_lead);
                        const color = getMqlColor(percent);
...

                {
                    ...
                    fixed: "left",
                    render: (_, row) => {
                        const percent = getSqlPercent(row.sql, row.mql);
            ...
            const firstKey = activeTab === "referral" ? "referral_url" : "tracking_url";
            ...
            const fixedColumns = filteredColumns.filter(col =>
                fixedKeys.includes(col.dataIndex)
            );
            ...
            setColumns(
                [...fixedColumns, ...sortedOtherColumns].map(col => ({
                    ...col,
                    key: `${col.dataIndex}_${activeTab}`
                }))
            );
    ...
    const totals = useMemo(() => {
        ...
        columns.forEach(col => {
            ...
            if (col.dataIndex === (activeTab === "referral" ? "referral_url" : "tracking_url")) return;
            ...
            <Spin spinning={loading}>
                ...
                <Tabs
                    ...
                    items={[
                        {
                            key: "tracking",
                            label: <strong>Tracking URL</strong>,
                        },
                        {
                            key: "referral",
                            label: <strong>Referral URL</strong>,
                        },
                    ]}
                />
                ...
                <Table
                    key={activeTab}
                    size="small"
                    rowKey={(row, index) => `${activeTab}_${index}`}
                    columns={columns}
                    dataSource={paginatedData}
                    pagination={false}
                    scroll={{ x: "max-content" }}
                    locale={{
                        emptyText: <Empty description="No Tracking URL Performance data available" />
                    }}
                    summary={() => (
                            ...
                            <Table.Summary.Row>
                                {columns.map((col, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {key === (activeTab === "referral" ? "referral_url" : "tracking_url") ? (
                                                <strong>Total</strong>
                                            ) : key?.includes("_pct") ? (
...
export default TrackingUrlPerformance;