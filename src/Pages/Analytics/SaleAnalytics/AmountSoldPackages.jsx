...
import { getSalesPackagesAmountAnalyticsService } from "../ApiService";
...
const AmountSoldPackages = () => {
  ...
  const fetchData = () => {
    ...
    getSalesPackagesAmountAnalyticsService(payload).then((response) => {
      if (response.data && response.data.data) {
        setStaffNames(response.data.data.map((item) => item.staff));
        setPackageAmount(response.data.data.map((item) => item.packages_amount));
      }
    });
  };
...
export default AmountSoldPackages;
